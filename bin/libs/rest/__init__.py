...
__repo__ = "https://github.com/srelab/falcon-plugin"
__site__ = "https://liusha.me"
__copyright__ = "Copyright (C) 2017 LiuSha"
__author__ = "LiuSha"
__license__ = "MIT"
__version__ = "0.1.0"
...
def cache_cleanup(max_age=None):
    ...
    :param int max_age: [opt] The max age the cache can be before removal.
                        defaults => :data:`MAX_AGE <rest.MAX_AGE>`
    """
...
class Session(ConnectionManager):
    ...
    :ivar int max_age: Max age the cache can be, before it’s considered stale. -1 will disable caching.
                       Defaults to :data:`MAX_AGE <rest.MAX_AGE>`
    """
    ...
    def get(self, url, params=None, **kwargs):
        ...
        :param kwargs: Optional arguments that :func:`request <rest.request>` takes.
...
class Response(object):
    ...
    def __init__(self, response, org_request, start_time, history):
        ...
        #: The :class:`Request <rest.Request>` object to which this is a response.
    ...
    @CachedProperty
    def encoding(self):
        """Encoding, to decode with, when accessing :meth:`resp.text <rest.Response.text>`."""
    ...
    def text(self):
        ...
        Will fallback to :data:`apparent_encoding <rest.Response.apparent_encoding>`
...
def metric(value, name, step=60, counter_type="GAUGE", tags=""):
    return {
        "endpoint": hostname(), "metric": name, "timestamp": timestamp(),
        "step": step, "value": value, "counterType": counter_type, "tags": tags
    }
